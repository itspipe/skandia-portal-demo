...
with st.sidebar:
    ...
    if logged:
        ...
        # Estado del caso
        for cond, bg, bc, txt in [
            (st.session_state.get("error_activo"),    "#FFF8E1","#F9A825", f"⚠️ {st.session_state.get('error_activo',{}).get('error_id','')} activo"),
            (st.session_state.get("tecnico_conectado"),"#EBF3FF","#1565C0","👨‍💻 Técnico conectado"),
            (st.session_state.get("caso_escalado"),    "#FFEBEE","#D32F2F", f"🚨 {st.session_state.get('ticket_actual',{}).get('numero','')}"),
            (st.session_state.get("caso_resuelto"),    "#E8F5E9","#00D261","✅ Caso resuelto"),
        ]:
            if cond:
                st.markdown(f'<div style="background:{bg};border-left:3px solid {bc};border-radius:8px;padding:7px 10px;font-size:12px;margin-bottom:6px;">{txt}</div>', unsafe_allow_html=True)
